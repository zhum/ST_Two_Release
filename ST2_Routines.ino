...
#define BPTIMEOUT 30000
#define paddle_width 4

int ball_x,ball_y,dx,dy,paddle_y;
...
void newFigure(){
  ...
  pos=3;
  height=0;
  putFigure(fig,pos,height,angle);
  nextMoveTime=currentMillis+tetrisSpeed;
}

int fig_can_move(uint8_t f,uint8_t p,uint8_t h,uint8_t a){
  uint8_t i,x,y;
  for(i=0;i<4;++i){
    x=h+figures[f][a][i][0];
    y=p+figures[f][a][i][1];
...
void putFigure(uint8_t f,uint8_t p,uint8_t h,uint8_t a){
  uint8_t i,x,y;
  for(i=0;i<4;++i){
    x=h+figures[f][a][i][0];
    y=p+figures[f][a][i][1];
...
void cleanFigure(uint8_t f,uint8_t p,uint8_t h,uint8_t a){
  uint8_t i,x,y;
  for(i=0;i<4;++i){
    x=h+figures[f][a][i][0];
    y=p+figures[f][a][i][1];
...
void arkanoid(){
  ...
  if(SUBSTATE==0){
    // init
    ball_x=ball_y=paddle_y=bpTime=0;
    dx=dy=1;
    clearmatrix();
    bpTime=0;
  }
  ...
  for(i=0;i<paddle_width;++i){
    LEDMAT[paddle_y+i] &= 63;
  }
  // erase ball
  LEDMAT[ball_y] &= ~(bit(ball_x));
  ...
  if(left_button==LOW){
    if(right_button==LOW){
      // check for quit...
      if(bpTime==0){
        bpTime=currentMillis+BPTIMEOUT;
        return;
      }
      else{
        if(currentMillis > bpTime){
          displayString("Bye!");
          STATE=0;
          delay(1000);
          return;
    ...
    // only left...
    paddle_y+=2;
    if(paddle_y+paddle_width>LED_HEIGHT){
      paddle_y=LED_HEIGHT-paddle_width;
    }
  }
  if(right_button==LOW){
    bpTime=0;
  ...
  // draw paddle
  for(i=0;i<paddle_width;++i){
    LEDMAT[paddle_y+i] |= 64;
  }

  // check borders
  if(ball_y==0)
    dy=1;
  if(ball_y==LED_HEIGHT-1)
    dy=-1;
  if(ball_x==0)
    dx=1;
  if(ball_x==6){
    // check for paddle
    if(ball_y>=paddle_y && ball_y<=paddle_y+paddle_width){
      dx=-1;
    }
    else{ //fail!!!!
      displayString("Oops");
...
void tetris(){
  uint8_t i,j,newangle=angle;
  NextSUBStateRequest=NextStateRequest=0;
  ...
  switch(SUBSTATE){
    ...
    case 1:
      ...
      if(left_button!=last_left){
        if(last_left==LOW){//left released
          if(right_button==HIGH){ //both were pressed
            if(bpTime>currentMillis){
              // quit!
              displayString("BYE!");
              return;             
            }
          ...
          else{ //only left was pressed, then released
            pos-=1;
            if(pos<0)
              pos=0;
          }
        }
        else{// left was just pressed
          if(right_button==HIGH)
            bpTime=currentMillis+quitTimeout;
        }
      }
      if(right_button!=last_right){
        if(last_right==LOW){//right released
          if(left_button==HIGH){ //both were pressed
            if(bpTime>currentMillis){
              // quit!
              NextState();
              return;             
          ...
          else{ //only right was pressed, then released
            pos+=1;
            if(pos>7)
              pos=7;
          }
      ...
      if(newangle!=angle){
        if(!fig_can_move(fig,pos,height,newangle)){
          newangle=angle;
        }
      }
      if (currentMillis>nextMoveTime){
        if (fig_can_move(fig,pos,height+1,newangle)){
          cleanFigure(fig,pos,height,angle);
          height+=1;
        }
        else{
          // stopped. Check full lines!
          for(i=LED_HEIGHT-1;i>0;--i){
            while(LEDMAT[i]==255){
              //scroll down image
              for(j=i;j>0;--j){
                LEDMAT[j]=LEDMAT[j-1];
